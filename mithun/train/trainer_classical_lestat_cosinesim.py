...
class MyCustomModel(PytorchModel):
    def __init__(self):
        super().__init__()
        self.net = torch.nn.Linear(2, LAST_LAYER_NEURON_COUNT)
...
train_labels, train_data_claim = read_data('../data/lestat_small_train_data_claim.txt')
train_labels, train_data_evidence = read_data('../data/lestat_small_train_data_evidence.txt')
val_labels, val_data = read_data('../data/lestat_small_dev_data.txt')
test_labels, test_data = read_data('../data/lestat_small_test_data.txt')
...
if(TYPE_OF_MODEL=='discocat'):
    ...
    train_diagrams_evidence = parser.sentences2diagrams(train_data_evidence)
    val_diagrams = parser.sentences2diagrams(val_data)
    test_diagrams = parser.sentences2diagrams(test_data)

if(TYPE_OF_MODEL=='spider'):
...
val_circuits =  [ansatz(diagram) for diagram in train_diagrams_claim]
...
all_circuits = train_circuits_claim +train_circuits_evidence+val_circuits + test_circuits
