...
1. Extract the word :term:`symbols <symbol>` from all diagrams in a set to create a vocabulary.
...
In the quantum case we do not explicitly have tensors, but :term:`circuit <quantum circuit>` parameters defining rotation angles on :term:`qubits <qubit>`, that need to be associated with concrete numbers; these are also represented by :term:`symbols <symbol>`. The first part of this tutorial provides a short introduction to :term:`symbols <symbol>` and their use, while in the second part we will go through all stages of a complete experiment.
