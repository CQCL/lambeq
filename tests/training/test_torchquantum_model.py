...
from lambeq import (AtomicType, Dataset, IQPAnsatz, TorchQuantumModel,
                    PytorchTrainer)
...
def test_init():
    ...
    model = TorchQuantumModel.from_diagrams(diagrams)
...
def test_checkpoint_loading():
    ...
    diagram = ansatz((Word("Alice", N) @ Word("runs", N >> S)
                      >> Cup(N, N.r) @ Id(S)))
    model = TorchQuantumModel.from_diagrams([diagram])
    model.initialise_weights()
    ...
    with patch('lambeq.training.checkpoint.open', mock_open(read_data=pickle.dumps(checkpoint))) as m, \
        ...
        model_new = TorchQuantumModel.from_checkpoint('model.lt')
...
def test_checkpoint_loading_errors():
    ...
    with patch('lambeq.training.checkpoint.open', mock_open(read_data=pickle.dumps(checkpoint))) as m, \
            patch('lambeq.training.checkpoint.os.path.exists', lambda x: True) as p:
        with pytest.raises(KeyError):
            _ = TorchQuantumModel.from_checkpoint('model.lt')
        m.assert_called_with('model.lt', 'rb')
...
def test_with_pytorch_trainer(tmp_path):
    ...
    model = TorchQuantumModel.from_diagrams(train_circuits + dev_circuits)
