...
import functools
from typing import Sequence, Iterable, TypeVar, Collection, Any
from tensornetwork import Edge, AbstractNode, get_subgraph_dangling, get_all_edges, contract_parallel, contract_between
...
ContractionKey =  tuple[tuple[tuple[str, ...], ...], tuple[str, ...], tuple[tuple[str, int], ...]]
...
def _dedup(seq: Iterable[T]) -> list[T]:
    ...
    """
    seen: set[T] = set()
    # Alias the add method so python doesn't have to check it each time we call it
    seen_add = seen.add
...
class TnOptimizer(oe.paths.PathOptimizer):
    ...
    def __init__(self, algorithm: str = "auto", memory_limit: int | None = None, **kwargs):
        """
        parameters
        ----------
        algorithm: :py:class:`str`
            ...
            - ``auto-hq``: slow, but finds very high quality paths. Not recommended.
            - ``random-greedy``: highly configurable, usually faster than auto-hq and finds better paths than auto.
        memory_limit: :py:class:`int`
            Limit the memory usage of the intermediate tensors. This is not recommended, as it will generally make the
            path finding much slower. If the size is a concern, use the random-greedy algorithm with ``minimize='size'``
            instead.
        kwargs: extra keyword arguments to pass to the fallback algorithm initializer.
            These will depend on the chosen fallback algorithm. For random-greedy, the following kwargs are available:

            - ``max_repeats``: int = 32
            - ``max_time``: float
            - ``minimize``: 'size' | 'flops' = 'flops'
            - ``parallel``: bool | int = False - whether to run trials in parallel. If a number is specified,
              use that many processes at once, otherwise use all available CPU cores.
        """
    ...
    def __call__(
        ...
        return self._optimizer(inputs, output, size_dict, memory_limit=memory_limit or self.memory_limit)
...
class SavedTnOptimizer(TnOptimizer):
    ...
    def __init__(self, algorithm: str = "auto-hq", **kwargs):
        ...
        ----------
        algorithm: :py:class:`str`
            fallback algorithm type to use when the path is not already cached.
            recommended options are:

            - ``auto``: fast, but paths are not that efficient. Use if you do not expect paths to be reused.
            - ``auto-hq`` (default): slow, but finds very high quality paths.
            - ``random-greedy``: highly configurable, usually faster than auto-hq and finds better paths than auto.
              Preferred option for larger networks where auto-hq is too slow.

        kwargs: extra keyword arguments to pass to the fallback algorithm initializer.
            These will depend on the chosen fallback algorithm. All options above accept a max memory kwarg:

            - ``memory_limit``: (optional) int

            However, it is preferable to use random-greedy with ``minimize='size'`` for better performance.
    ...
    def __call__(
        ...
        if edge_list is None:
            raise ValueError("Edge list must be supplied in order to cache paths.")

        # Tensornetwork gives us edges with wacky names and arbitrary ordering.
        # Make them a single symbol instead for consistency starting from 'a' for the first input item.
        edge_map = {
        ...
        if key not in self.saved_paths:
            # Couldn't find a saved path. Use fallback and save it for next time.
            path = super().__call__(inputs, output, size_dict, memory_limit, edge_list)
...
def custom_contractor(
    ...
    path, nodes = _get_path(_dedup([n for n in nodes if n in nodes_set]), algorithm)
    for a, b in path:
...
def _get_path(
    ...
    A copy of the tensornetwork implementation, that uses a consistent node ordering.
    ...
    return algorithm(input_sets, output_set, size_dict, edge_list=edge_list), nodes
