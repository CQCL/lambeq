...
class NelderMeadOptimizer(Optimizer):
    ...
    The Nelder-Mead optimizer is an algorithm used for unconstrained optimization in 
    multidimensional spaces. Unlike some other optimization methods, it does not take 
    into account any bounds or constraints on the variables. The algorithm is based 
    on the Simplex method and is particularly useful when the derivatives 
    (first and second) of the objective function are unknown or unreliable.

    Although the Nelder-Mead algorithm is generally robust and widely applicable, 
    it has some limitations. In cases where the derivatives can be accurately 
    computed, alternative algorithms that utilize this derivative information 
    may offer better performance. These methods are often preferred due to 
    their ability to handle a wider range of scenarios and their tendency 
    to converge to more optimal solutions. It is worth noting that the Nelder-Mead 
    technique is a heuristic search approach, which means that it can sometimes 
    ...
    def __init__(self, model: QuantumModel,
        ...
        The hyperparameters may contain the following key value pairs::
            ...
            hyperparams = {
                'adpative': Adjust the algorithm's parameters based on the dimensionality of the problem. 
                			This adaptation is particularly helpful when minimizing functions in 
                            high-dimensional spaces, bool.
                            
                'initial_simplex': If provided, the `initial simplex` replaces the initial model weights. 
                				   Each row initial_simplex[i, :] should contain the coordinates of the 
                                   ith vertex among the N+1 vertices in the simplex, where N represents 
                                   the dimension, ArrayLike (N+1, N), float.
        
                'xatol': The acceptable level of absolute error in the optimal model weights (optimal solution) 
                		 between iterations that indicates convergence, float.
                         
                'fatol': The acceptable level of absolute error in the loss value between iterations that 
                         indicates convergence, float.
            }
        ...
        if bounds is None:
            ...
            # check bounds
            if (lb > ub).any():
                raise ValueError("Nelder Mead - lower bounds must be less than upper bounds.")
            if np.any(lb > self.model.weights) or np.any(self.model.weights > ub):
                warnings.warn("Initial value of model weights is not within the bounds.")
        ...
        if self.initial_simplex is None:
            ...
            if self.sim.ndim != 2 or self.sim.shape[0] != self.sim.shape[1] + 1:
                raise ValueError(f"`initial_simplex` should be an array of shape ({N+1},{N})")
            if len(self.model.weights) != self.sim.shape[1]:
                raise ValueError(f"Size of `initial_simplex` is not consistent with `model.weights`")
            N = self.sim.shape[1]
    ...
    def backward(
        ...
        try:
            if (np.max(np.ravel(np.abs(self.sim[1:] - self.sim[0]))) <= self.xatol and
                np.max(np.abs(self.fsim[0] - self.fsim[1:])) <= self.fatol):
                raise ValueError("xtol and ftol termination conditions are satisfied.")
            ...
            if fxr < self.fsim[0]:
                xe = (1 + self.rho * self.chi) * xbar - self.rho * self.chi * self.sim[-1]
                xe = self.project(xe)
                ...
                if fxr < self.fsim[-2]:
                    ...
                    # Perform contraction
                    if fxr < self.fsim[-1]:
                        xc = (1 + self.psi * self.rho) * xbar - self.psi * self.rho * self.sim[-1]
                        xc = self.project(xc)
                    ...
                    if doshrink:
                        for j in self.one2np1:
                            self.sim[j] = self.sim[0] + self.sigma * (self.sim[j] - self.sim[0])
                            self.sim[j] = self.project(self.sim[j])
                            self.fsim[j] = self.func(diagrams, targets, self.sim[j])
        except:
            pass
        finally:
            self.ind = np.argsort(self.fsim)
